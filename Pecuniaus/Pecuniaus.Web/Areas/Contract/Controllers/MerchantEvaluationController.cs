...
{
    public class MerchantEvaluationController : BaseController
    {
        public ActionResult Index()
        {
            var apiMethodProcessor = string.Format("creditreport/information/" + CurrentMerchantID + "?contractId=" + ContractID);
            var model= BaseApiData.GetAPIResult<MerchantScoreInfo>(apiMethodProcessor, () => new MerchantScoreInfo()) ?? new MerchantScoreInfo();
            return View("Index", model);
            //var apiMethodProcessor = string.Format("creditreport/score/" + CurrentMerchantID + "/" + ContractID + "?workflowid=" + WorkflowID + "&tasktypeid=0");
        ...
        //            if (submit == "Complete")
        //            {
        //                var apiMethod = string.Format("creditreport/save?tasktypeid={0}&merchantId={1}&contractId={2}&iscompleted={3}&workflowid={4}", 5, CurrentMerchantID, ContractID, "1", 1);
        ...
        //            else if (submit == "Save")
        //            {
        //                var apiMethod = string.Format("creditreport/save?tasktypeid={0}&merchantId={1}&contractId={2}&iscompleted={3}&workflowid={4}", 5, CurrentMerchantID, ContractID, "0", 1);
        //                var response = BaseApiData.PostAPIData(apiMethod, model);
        //                if (response.StatusCode == System.Net.HttpStatusCode.OK)
        //                {
        ...
        {
            return PartialView("_CreditPull", new CreditPull
            {
                Type = GetPullType()
            });
        }
        ...
        //    var modelProcessor = BaseApiData.GetAPIResult<List<CreditReport>>(apiMethodProcessor, () => new List<CreditReport>());
        //    List<CreditReport> listcreditreport = modelProcessor;
        //    List<CreditReport> volumelistFiltered = listcreditreport.Where(c => c.creditReportId == creditReportId).ToList();
        ...
        public ActionResult ListCreditReport()
        {
            ...
            return PartialView("_ListCreditReport", new List<CreditReport>());
        }
        ...
        //    var model = BaseApiData.PutAPIData<CreditReport>(apiMethod, new CreditReport());
        ...
        /// </summary>
        /// <returns></returns>
        private IEnumerable<SelectListItem> GetPullType()
        {
        ...
        //    var apiMethodProcessor = string.Format("creditreport/" + CurrentMerchantID + "/" + ContractID);
        //    var reports = BaseApiData.GetAPIResult<List<CreditReport>>(apiMethodProcessor, () => new List<CreditReport>());
